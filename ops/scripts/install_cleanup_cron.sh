...
CRON_FILE="${1:-${INFRA_ROOT}/ops/cron/stellcodex-cleanup.cron}"
LOGROTATE_SRC="${LOGROTATE_SRC:-${INFRA_ROOT}/ops/logrotate/stellcodex-runtime.conf}"
...
grep -Fv "/srv/infra/ops/scripts/cleanup.sh" "${TMP}" \
  | grep -Fv "/srv/infra/ops/scripts/backup-state.sh" \
